...
import { X, Wallpaper, Leaf, Circle, Settings, Shirt, User, Image, Video } from "lucide-react";
...
const PreviewModal = ({ imageUrls, attachmentIds, onClose, supportsVideo }: PreviewProps) => {
    ...
    <div className="ai-enhancer-modal fixed inset-[0px] bg-[rgba(0,0,0,0.5)] flex items-center justify-center z-[200000]">
      <div className="ai-enhancer-modal__content bg-[#fff] rounded-[3px] max-w-[1000px] w-[90vw] h-auto">
        <div className="flex flex-row justify-between border-b-[1px] border-b-[#E9E9E9] pt-[16px] pl-[24px] pr-[24px]">
          <h2 className="mt-0">{ __( 'AI Product Image Generation', 'tryaura' ) }</h2>
          <button className="w-[16px] h-[16px] cursor-pointer" onClick={onClose} aria-label="Close modal">
            <X size={ 16 } />
          </button>
        </div>
        ...
        <div className="flex flex-row gap-[32px] mt-[27px] pl-[24px] pr-[24px]">
          <div className="w-[500px] max-h-[533px] overflow-auto">
            <div className="text-[14px] mb-[8px]">{multiple ? 'Original Images' : 'Original Image'}</div>
            {multiple ? (
              <div className="flex flex-col gap-[8px]">
                {imageUrls.map((url, idx) => (
                  <img key={idx} src={url} alt={`Original ${idx + 1}`} className="w-auto h-[152px] h-auto block border-none rounded-[8px]" />
                ))}
              </div>
            ) : (
              <img src={imageUrls[0]} alt="Original" className="w-auto h-[152px] h-auto block border-none rounded-[8px]" />
            )}
          </div>
          <div className="w-full flex flex-col gap-[32px]">
            {/* Tabs for Generated content */}
            {
              supportsVideo && (
                <GroupButton
                  options={
                    [
                      {label: __( 'Generate Image', 'tryaura' ), value: 'image'},
                      {label: __( 'Generate Video', 'tryaura' ), value: 'video', disabled: !generatedUrl },
                    ]
                  }
                  value={activeTab}
            ...
            <div className="flex flex-col gap-[12px]">
              ...
              <ModernSelect
                ...
                options={[
                  { label: 'Plain', value: __( 'plain', 'tryaura' ), icon: <Circle/> },
                  { label: 'Natural', value: __( 'natural', 'tryaura' ), icon: <Leaf/> },
                  { label: 'Studio', value: __( 'studio', 'tryaura' ), icon: <Wallpaper/> },
                  { label: 'Custom', value: __( 'custom', 'tryaura' ), icon: <Settings/> },
                ]}
              />
              ...
              <ModernSelect
                ...
                options={[
                  { label: 'Photo realistic', value: __( 'photo-realistic', 'tryaura' ), icon: <Image/> },
                  { label: 'Studio mockup', value: __( 'studio mockup', 'tryaura' ), icon: <Shirt/> },
                  { label: 'Model shoot', value: __( 'model shoot', 'tryaura' ), icon: <User/> },
                ]}
              />

              <label style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                <span className="w-[500] text-[14px] mb-[8px]">{ __( 'Prompt (Optional)' ) }</span>
                <textarea className="border border-[#E9E9E9]" value={optionalPrompt} onChange={(e: any) => setOptionalPrompt(e.target.value)} rows={3} placeholder={ __( 'Add any specific instructions (optional)', 'tryaura' ) } />
              </label>
              ...
              <div className="flex flex-row gap-[12px]">
                {generatedUrl ? (
                  <>
                    ...
                    >
                      {
                        isBusy ? __( 'Regenerating…', 'tryaura' ) : __( 'Regenerate', 'tryaura' )
                      }
                    </Button>
                  ...
                  >
                    {
                      isBusy ? __( 'Generating…', 'tryaura' ) : __( 'Generate', 'tryaura' )
                    }
                  </Button>
                )}
              </div>
          ...
          <div className="w-full">
            <div className="w-[500] text-[14px] mb-[8px]">{ __( 'Generated Output', 'tryaura' ) }</div>
            {activeTab === 'image' ? (
              generatedUrl ? (
                <div className="flex flex-col gap-[20px]">
                  <img src={generatedUrl} alt="Generated" className="w-full h-auto block rounded-[8px]" />
                  {
                    supportsVideo && (
                      <div className="flex justify-center">
                        <Button variant="solid" className="border border-primary text-primary bg-white" onClick={() => setActiveTab('video')}>
        ...
        <div className="mt-[24px] border-t-[1px] border-t-[#E9E9E9] flex flex-row justify-end p-[16px_24px] gap-[12px]">
          {
            generatedUrl &&
            <Button onClick={setInMediaSelection} disabled={isBusy || uploading || !generatedUrl}>
              {uploading ? __( 'Adding...' ) : __( 'Add to Media Library', 'tryaura' )}
            </Button>
          }
