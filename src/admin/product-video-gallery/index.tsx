...
(function ($) {
	$(function () {
			...
			const processImage = ($image, attachmentId) => {
				...
				const videoData =
					...
					tryAuraVideo.videoData[attachmentId]
						? JSON.stringify(tryAuraVideo.videoData[attachmentId])
						: '';
		...
		const addGlobalVideoButton = () => {
			...
			if ($container.length && !$('#tryaura-add-global-video').length) {
				const $btnWrapper = $(
		...
		$('body').on('click', '.tryaura-product-gallery-video', function (e) {
			e.preventDefault();
			const $btn = $(this);
			...
			renderModal(
				...
				async (newData) => {
					...
					if (newData && !newData.useCustomThumbnail && newData.url) {
						// Generate thumbnail via REST API
						try {
							const response = await fetch(
								`${tryAuraVideo.restUrl}tryaura/v1/generate-thumbnail`,
					...
					if (String(targetId) !== String(attachmentId)) {
						...
						if (index !== -1) {
							...
							$galleryInput.val(ids.join(',')).trigger('change');
						}
						$parentLi
						...
						$input.attr('name', `tryaura_video_data[${targetId}]`);
					...
					if (!newData || Object.keys(newData).length === 0) {
						...
						if (newData.useCustomThumbnail && thumbnailUrl) {
							$img.attr('src', thumbnailUrl).removeAttr('srcset');
							if ($parentLi.length) {
								$img.attr(
									'sizes',
									'auto, (max-width: 150px) 100vw, 150px'
