...
const TryOnModal = ({ productImages, onClose }: TryOnModalProps) => {
	...
	const [message, setMessage] = useState<string>(
		__('Select or capture one or more photos, then click Try On', 'tryaura')
	);
	...
	const startCamera = async () => {
		try {
			...
			setMessage(
				__('Camera active — click Capture when ready', 'tryaura')
			);
			setError(null);
	...
	const stopCamera = () => {
		...
		doAction('tryaura.after_camera_stop', videoRef, streamRef, canvasRef);
		const video = videoRef.current;
		if (video) {
			try {
				video.pause?.();
